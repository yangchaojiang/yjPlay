...
public final class DefaultCacheUtil {
    ...
    /**
     * Sets a {@link CacheUtil.CachingCounters} to contain the number of bytes already downloaded and the
     * length for the content defined by a {@code dataSpec}. {@link CacheUtil.CachingCounters#newlyCachedBytes}
     * is reset to 0.
     *
     * @param dataSpec Defines the data to be checked.
     * @param cache A {@link Cache} which has the data.
     * @param counters The {@link CacheUtil.CachingCounters} to update.
     */
    public static void getCached(DataSpec dataSpec, Cache cache, DefaultCacheUtil.CachingCounters counters) {
    ...
    public static void cache(
            ...
            int priority,
            @Nullable DefaultCacheUtil.CachingCounters counters,
            boolean enableEOFException,Downloader.ProgressListener listener, DefaultProgressDownloader defaultProgressDownloader)
        ...
        if (counters != null) {
            ...
            counters = new DefaultCacheUtil.CachingCounters();
        }
        ...
        while (left != 0) {
            ...
            if (blockLength > 0) {
                ...
                long read = readAndDiscard(dataSpec, start, blockLength, dataSource, buffer,
                        priorityTaskManager, priority, counters,listener,defaultProgressDownloader);
                if (read < blockLength) {
    ...
    private static long readAndDiscard(DataSpec dataSpec, long absoluteStreamPosition, long length,
                                       ...
                                       DefaultCacheUtil.CachingCounters counters,Downloader.ProgressListener listener, DefaultProgressDownloader downloader) throws IOException, InterruptedException {
        while (true) {
